...
int main(int argc,char *argv[])
{
	...
	FILE *fo = fopen("C:\TURBOC3\BIN\programs\lena_copy.bmp","wb"); // Output File name
	clock_t start, stop;
	FILE *streamIn;
	int i;
	int count = 0;
	...
	unsigned char buf[height * width]; // to store the image data	start=clock(); // Note the start time for profiling purposes.
